...
export default function AppAdmin() {
  ...
  useEffect(function() {
    supabase.auth.getSession().then(function(res) {
      setSession(res.data.session); setLoading(false)
    })
  ...
  if (!session) return (
    <div className="app-shell">
      <div className="emp-header admin-header">
        <div className="emp-header-logo"><KronikLogo /></div>
        <div>
          <div className="emp-header-title">Kron<span style={{color:'#F59E0B'}}>ik</span></div>
          <div className="emp-header-sub">Demo · Panel Admin</div>
        </div>
      </div>
  ...
  return (
    <div className="app-shell">
      ...
      <nav className="bottom-nav" style={{gridTemplateColumns:'repeat(8,1fr)'}}>
        {TABS.map(function(t) {
          return (
...
function UserIcon()   { return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> }
